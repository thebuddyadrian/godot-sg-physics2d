...
const real_t real_pi = 3.1415926535898;
...
namespace sine {
static real_t in(real_t t, real_t b, real_t c, real_t d) {
	return -c * cos(t / d * (real_pi / 2)) + c + b;
}

static real_t out(real_t t, real_t b, real_t c, real_t d) {
	return c * sin(t / d * (real_pi / 2)) + b;
}

static real_t in_out(real_t t, real_t b, real_t c, real_t d) {
	return -c / 2 * (cos(real_pi * t / d) - 1) + b;
}
...
static real_t in(real_t t, real_t b, real_t c, real_t d) {
	...
	float s = p / 4;
	float postFix = a * pow(2, 10 * (t -= 1)); // this is a fix, again, with post-increment operators
	return -(postFix * sin((t * d - s) * (2 * real_pi) / p)) + b;
}
...
static real_t out(real_t t, real_t b, real_t c, real_t d) {
	...
	float a = c;
	float s = p / 4;
	return (a * pow(2, -10 * t) * sin((t * d - s) * (2 * real_pi) / p) + c + b);
}
...
static real_t in_out(real_t t, real_t b, real_t c, real_t d) {
	...
	if (t < 1) {
		float postFix = a * pow(2, 10 * (t -= 1)); // postIncrement is evil
		return -0.5f * (postFix * sin((t * d - s) * (2 * real_pi) / p)) + b;
	}
	float postFix = a * pow(2, -10 * (t -= 1)); // postIncrement is evil
	return postFix * sin((t * d - s) * (2 * real_pi) / p) * 0.5f + c + b;
}
...
SGTween::real_interpolater SGTween::real_interpolaters[SGTween::TRANS_COUNT][SGTween::EASE_COUNT] = {
	{ &linear::in, &linear::out, &linear::in_out, &linear::out_in },
	{ &sine::in, &sine::out, &sine::in_out, &sine::out_in },
	{ &quint::in, &quint::out, &quint::in_out, &quint::out_in },
	{ &quart::in, &quart::out, &quart::in_out, &quart::out_in },
...
real_t SGTween::_run_equation(TransitionType p_trans_type, EaseType p_ease_type, real_t t, real_t b, real_t c, real_t d) {
	if (d == 0) {
		// Special case to avoid dividing by 0 in equations.
		return b + c;
	}

	real_interpolater cb = real_interpolaters[p_trans_type][p_ease_type];
	ERR_FAIL_COND_V(cb == NULL, b);
	return cb(t, b, c, d);
}